...
namespace Verpha.HierarchyDesigner
{
    public static class HierarchyDesigner_Manager_GameObject
    {
        ...
        private static void DrawGameObjectMainIcon(GameObject gameObject, Rect selectionRect, int instanceID)
        {
            ...
            if (!gameObject.activeInHierarchy) { GUI.color = inactiveColor; }
        ...
        #region Component Icons Methods
        private static void DrawGameObjectComponentIcons(GameObject gameObject, Rect selectionRect, int instanceID)
        {
            ...
            if (layoutMode == HierarchyDesigner_Configurable_GeneralSettings.HierarchyLayoutMode.Consecutive)
            {
                iconOffset = selectionRect.x + (GUI.skin.label.CalcSize(new GUIContent(gameObject.name)).x) + iconAdditionalOffset;
            }
            else if (layoutMode == HierarchyDesigner_Configurable_GeneralSettings.HierarchyLayoutMode.Split)
            {
                if (enableHierarchyButtons)
                {
                    iconOffset += lockButtonWidth + activeButtonWidth + 5f;
                }
            ...
            foreach ((Component component, Texture2D icon) in cachedIcons)
            {
                ...
                if (!gameObject.activeInHierarchy || isComponentDisabled) { GUI.color = inactiveColor; }
                else { GUI.color = activeColor; }
        ...
        private static bool CheckComponentsChanged(GameObject gameObject, List<(Component component, Texture2D icon)> cachedIcons)
        {
            ...
            if (currentComponents.Length != cachedIcons.Count)
            {
                return true;
            }
        ...
        private static void DrawHierarchyTree(GameObject gameObject, Rect selectionRect, int instanceID)
        {
            ...
            if (!gameObject.activeInHierarchy) { GUI.color = inactiveColor; }
            else { GUI.color = hierarchyTreeColor; }
        ...
        private static void DrawParentTreeFillLines(Transform transform, float rectX, float offsetX, float rectHeight, float rectY)
        {
            ...
            while (parentTransform != null)
            {
                ...
                if (!isLastSibling)
                {
                    GUI.DrawTexture(new Rect(rectX, rectY, rectHeight, rectHeight), treeBranchIconI, ScaleMode.ScaleToFit);
                }
        ...
        private static void DrawGameObjectTag(GameObject gameObject, Rect selectionRect, int instanceID)
        {
            ...
            if (layoutMode == HierarchyDesigner_Configurable_GeneralSettings.HierarchyLayoutMode.Docked)
            {
                if (enableHierarchyButtons)
                {
                    iconOffset += lockButtonWidth + activeButtonWidth;
                }
                if (enableGameObjectLayer)
                {
                    string layer;
        ...
        private static void DrawGameObjectLayer(GameObject gameObject, Rect selectionRect, int instanceID)
        {
            ...
            if (layoutMode == HierarchyDesigner_Configurable_GeneralSettings.HierarchyLayoutMode.Docked)
            {
                if (enableHierarchyButtons)
                {
                    selectionRect.x = EditorGUIUtility.currentViewWidth - lockButtonWidth - activeButtonWidth - layerLabelWidth - additionalLockLabelWidth;
                }
                else
                {
                    selectionRect.x = EditorGUIUtility.currentViewWidth - layerLabelWidth - 5;
                }
        ...
        #region Hierarchy Buttons Methods
        ...
        private static float CalculateButtonsRect(GameObject gameObject, Rect selectionRect)
        {
            if (layoutMode == HierarchyDesigner_Configurable_GeneralSettings.HierarchyLayoutMode.Consecutive)
            {
                float offsetX = selectionRect.x + 8f; // make this a design setting/
        ...
        #region Major Shortcuts Methods
        ...
        private static void ProcessRenameMajorShortcut()
        {
            ...
            HierarchyDesigner_Window_RenameTool.OpenWindow(selectedGameObjects, true);
        }
        #endregion
        ...
        private static void DrawGameObjectLock(GameObject gameObject, Rect selectionRect)
        {
            ...
            if (layoutMode == HierarchyDesigner_Configurable_GeneralSettings.HierarchyLayoutMode.Docked)
            {
                if (enableHierarchyButtons)
                {
                    selectionRect.x = EditorGUIUtility.currentViewWidth - lockButtonWidth - activeButtonWidth - lockLabelWidth - additionalLockLabelWidth - dockedPropertiesBaseOffset;
                }
                else
                {
                    selectionRect.x = EditorGUIUtility.currentViewWidth - lockLabelWidth - dockedPropertiesBaseOffset;
                }
            }
            else
            {
        ...
        private static void DrawFolder(GameObject gameObject, Rect selectionRect, int instanceID)
        {
            ...
            {
                HierarchyDesigner_Configurable_Folder.HierarchyDesigner_FolderData folderData = HierarchyDesigner_Configurable_Folder.GetFoldersData(gameObject.name) ?? new HierarchyDesigner_Configurable_Folder.HierarchyDesigner_FolderData();
            ...
            if (!gameObject.activeInHierarchy) { GUI.color = inactiveColor; }
            else { GUI.color = folderInfo.folderColor; }
