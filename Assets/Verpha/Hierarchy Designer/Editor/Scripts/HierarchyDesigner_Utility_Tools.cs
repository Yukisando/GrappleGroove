...
namespace Verpha.HierarchyDesigner
{
    public static class HierarchyDesigner_Utility_Tools
    {
        ...
        private static void Count_AllGameObjects()
        {
            ...
            Debug.Log($"Total <GameObjects> in the scene: {count}\nAll <GameObjects> found: {gameObjectNames}.\n");
        }
        ...
        private static void Count_AllParentGameObjects()
        {
            ...
            gameObjectNames = count == 0 ? "none" : gameObjectNames.TrimEnd(',', ' ');
            Debug.Log($"Total <Parent GameObjects> in the scene: {count}\nAll <Parent GameObjects> found: {gameObjectNames}.\n");
        }
        ...
        private static void Count_AllEmptyGameObjects()
        {
            ...
            gameObjectNames = count == 0 ? "none" : gameObjectNames.TrimEnd(',', ' ');
            Debug.Log($"Total <Empty GameObjects> in the scene: {count}\nAll <Empty GameObjects> found: {gameObjectNames}.\n");
        }
        ...
        private static void Count_AllLockedGameObjects()
        {
            ...
            gameObjectNames = count == 0 ? "none" : gameObjectNames.TrimEnd(',', ' ');
            Debug.Log($"Total <Locked GameObjects> in the scene: {count}\nAll <Locked GameObjects> found: {gameObjectNames}.\n");
        }
        ...
        private static void Count_AllActiveGameObjects()
        {
            ...
            gameObjectNames = count == 0 ? "none" : gameObjectNames.TrimEnd(',', ' ');
            Debug.Log($"Total <Active GameObjects> in the scene: {count}\nAll <Active GameObjects> found: {gameObjectNames}.\n");
        }
        ...
        private static void Count_AllInactiveGameObjects()
        {
            ...
            gameObjectNames = count == 0 ? "none" : gameObjectNames.TrimEnd(',', ' ');
            Debug.Log($"Total <Inactive GameObjects> in the scene: {count}\nAll <Inactive GameObjects> found: {gameObjectNames}.\n");
        }
        ...
        private static void Count_AllFolders()
        {
            ...
            gameObjectNames = count == 0 ? "none" : gameObjectNames.TrimEnd(',', ' ');
            Debug.Log($"Total <Folders> in the scene: {count}\nAll <Folders> found: {gameObjectNames}.\n");
        }
        ...
        private static void Count_AllSeparators()
        {
            ...
            foreach (GameObject gameObject in allGameObjects)
            {
                if (IsGameObjectSeparator(gameObject))
                {
                    count++;
                    gameObjectNames += gameObject.name + ", ";
                }
            ...
            gameObjectNames = count == 0 ? "none" : gameObjectNames.TrimEnd(',', ' ');
            Debug.Log($"Total <Separators> in the scene: {count}\nAll <Separators> found: {gameObjectNames}.\n");
        }
        ...
        private static void Count_AllPhysicsDynamicSprites()
        {
            ...
            gameObjectNames = count == 0 ? "none" : gameObjectNames.TrimEnd(',', ' ');
            Debug.Log($"Total 2D <Dynamic> physics sprites in the scene: {count}\nAll <Dynamic> sprites found: {gameObjectNames}.\n");
        }
        ...
        private static void Count_AllPhysicsStaticSprites()
        {
            ...
            gameObjectNames = count == 0 ? "none" : gameObjectNames.TrimEnd(',', ' ');
            Debug.Log($"Total 2D <Static> physics sprites in the scene: {count}\nAll <Static> sprites found: {gameObjectNames}.\n");
        }
        ...
        private static void Count_All3DObjectsByType(string objectType)
        {
            ...
            gameObjectNames = count == 0 ? "none" : gameObjectNames.TrimEnd(',', ' ');
            Debug.Log($"Total 3D <{objectType}> objects in the scene: {count}\nAll <{objectType}> sprites found: {gameObjectNames}.\n");
        }
        ...
        private static void Count_AllHalos()
        {
            ...
            gameObjectNames = count == 0 ? "none" : gameObjectNames.TrimEnd(',', ' ');
            Debug.Log($"Total <Halos> in the scene: {count}\nAll <Halos> found: {gameObjectNames}.\n");
        }
        ...
        private static void Rename_SelectedGameObjects(string sortingActionDescription, bool autoIndex)
        {
            ...
            HierarchyDesigner_Window_RenameTool.OpenWindow(selectedGameObjects, autoIndex);
        }

        private static void Rename_OpenRenameToolWindow()
        {
            HierarchyDesigner_Window_RenameTool.OpenWindow(null, true);
        }
        #endregion
        #endregion
        ...
        {
            IEnumerable<GameObject> allGameObjects = GetAllGameObjectsInActiveScene();
            List<GameObject> gameObjectsList = new List<GameObject>(allGameObjects);
            SelectOrDisplayMessage(gameObjectsList, "GameObjects");
        }
        ...
        private static void Select_AllParentGameObjects()
        {
            ...
            foreach (GameObject gameObject in allGameObjects)
            {
                if (IsGameObjectParent(gameObject)) { parentGameObjects.Add(gameObject); }
            }
            SelectOrDisplayMessage(parentGameObjects, "Parent GameObjects");
        }
        ...
        private static void Select_AllEmptyGameObjects()
        {
            ...
            foreach (GameObject gameObject in allGameObjects)
            {
                if (IsGameObjectEmpty(gameObject)) { emptyGameObjects.Add(gameObject); }
            }
            SelectOrDisplayMessage(emptyGameObjects, "Empty GameObjects");
        }

        private static void Select_AllLockedGameObjects()
        {
            IEnumerable<GameObject> allGameObjects = GetAllGameObjectsInActiveScene();
            List<GameObject> lockedGameObjects = new List<GameObject>();
            foreach (GameObject gameObject in allGameObjects)
            {
                if (IsGameObjectLocked(gameObject)) { lockedGameObjects.Add(gameObject); }
            }
            SelectOrDisplayMessage(lockedGameObjects, "Locked GameObjects");
        }
        ...
        private static void Select_AllActiveGameObjects()
        {
            ...
            foreach (GameObject gameObject in allGameObjects)
            {
                if (IsGameObjectActive(gameObject)) { activeGameObjects.Add(gameObject); }
            }
            SelectOrDisplayMessage(activeGameObjects, "Active GameObjects");
        }
        ...
        private static void Select_AllInactiveGameObjects()
        {
            ...
            foreach (GameObject gameObject in allGameObjects)
            {
                if (!IsGameObjectActive(gameObject)) { inactiveGameObjects.Add(gameObject); }
            }
            SelectOrDisplayMessage(inactiveGameObjects, "Inactive GameObjects");
        }
        ...
        private static void Select_AllFolders()
        {
            ...
            foreach (GameObject gameObject in allGameObjects)
            {
                if (IsGameObjectFolder(gameObject)) { folders.Add(gameObject); }
            }
            SelectOrDisplayMessage(folders, "Folders");
        }
        ...
        private static void Select_AllSeparators()
        {
            ...
            foreach (GameObject gameObject in allGameObjects)
            {
                if (IsGameObjectSeparator(gameObject)) { separators.Add(gameObject); }
            }
            SelectOrDisplayMessage(separators, "Separators");
        }
        ...
        private static void Select_AllComponentOfType<T>() where T : Component
        {
            ...
            SelectOrDisplayMessage(gameObjectsWithComponent, typeof(T).Name);
        }
        ...
        private static void Select_All2DSpritesByType(string spriteType)
        {
            ...
            SelectOrDisplayMessage(selectedSprites, spriteType);
        }

        private static void Select_AllPhysicsDynamicSprites()
        {
            IEnumerable<GameObject> allGameObjects = GetAllGameObjectsInActiveScene();
            List<GameObject> dynamicSprites = new List<GameObject>();
            foreach (GameObject gameObject in allGameObjects)
            {
                SpriteRenderer spriteRenderer = gameObject.GetComponent<SpriteRenderer>();
                Rigidbody2D rigidbody2D = gameObject.GetComponent<Rigidbody2D>();
                if (spriteRenderer != null && rigidbody2D != null && rigidbody2D.bodyType == RigidbodyType2D.Dynamic)
                {
                    dynamicSprites.Add(gameObject);
                }
            }
            SelectOrDisplayMessage(dynamicSprites, "Dynamic Sprites");
        }

        private static void Select_AllPhysicsStaticSprites()
        {
            IEnumerable<GameObject> allGameObjects = GetAllGameObjectsInActiveScene();
            List<GameObject> staticSprites = new List<GameObject>();
            foreach (GameObject gameObject in allGameObjects)
            {
                SpriteRenderer spriteRenderer = gameObject.GetComponent<SpriteRenderer>();
                Rigidbody2D rigidbody2D = gameObject.GetComponent<Rigidbody2D>();
                if (spriteRenderer != null && rigidbody2D != null && (rigidbody2D.bodyType == RigidbodyType2D.Static || rigidbody2D.bodyType == RigidbodyType2D.Kinematic))
                {
                    staticSprites.Add(gameObject);
                }
            }
            SelectOrDisplayMessage(staticSprites, "Static Sprites");
        }
        ...
        private static void Select_All3DObjectsByType(string objectType)
        {
            ...
            foreach (GameObject gameObject in allGameObjects)
            {
                ...
                if (meshFilter != null && meshFilter.sharedMesh != null && meshFilter.sharedMesh.name.Contains(objectType))
                {
                    objectsOfType.Add(gameObject);
                }
            }
            SelectOrDisplayMessage(objectsOfType, objectType);
        }

        private static void Select_AllHalos()
        {
            IEnumerable<GameObject> allGameObjects = GetAllGameObjectsInActiveScene();
            List<GameObject> haloObjects = new List<GameObject>();
            foreach (GameObject gameObject in allGameObjects)
            {
                Behaviour halo = (Behaviour)gameObject.GetComponent("Halo");
                if (halo != null)
                {
                    haloObjects.Add(gameObject);
                }
            }
            SelectOrDisplayMessage(haloObjects, "Halos");
        }
        ...
        private static void Sort_GameObjectChildren(Comparison<GameObject> comparison, string sortingActionDescription)
        {
            ...
            if (selectedGameObject == null) { Debug.Log($"No Parent GameObject selected. Please select one Parent GameObject to {sortingActionDescription}."); return; }
