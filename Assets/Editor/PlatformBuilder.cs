...
static class PlatformBuilder
{
    ...
    static void AllWindowsZipped() {
        ...
        string appName = "Bundle_zipped.exe";
        string buildPath = Path.GetFullPath("./Builds/Windows/Bundle/");
        BuildPipeline.BuildPlayer(EditorBuildSettings.scenes, buildPath + appName, BuildTarget.StandaloneWindows64, BuildOptions.None);
    ...
    static void CreateWinRARZip(string buildPath, string appName) {
        string zipFileName = $"{Path.GetFileNameWithoutExtension(appName)}.zip";
        string zipPath = Path.Combine(buildPath, zipFileName);
        string winRarPath = @"C:\Program Files\WinRAR\WinRAR.exe";
        
        string cmdArguments = $"a -m1 \"{zipPath}\" \"*.*\" -r";
